...
global iline, icts, lp3a, lp3b, l1, l2, l3, l4, l5
...
def fwhm_fit2(aplist,targs,is_first_iter):   
    nlc = len(targs[:,0])
    fwhm_vec = []
    ...
    if is_first_iter:
        ...
        ax1 = fig.add_subplot()
        lp3a, = plot(aplist_shifted,dapvec2,'o')
        lp3b, = plot(apfine,psf_fit,'-')
        ...
        lp3a.set_xdata(aplist_shifted)
        lp3a.set_ydata(dapvec2)
        lp3b.set_xdata(apfine)
        lp3b.set_ydata(psf_fit)
...
def applot(apfile, aplist,sigvec,apmin,cstring,is_first_iter):
    #apfile = 'aperture.pdf'
    #fig=figure(2)
    ...
    if is_first_iter:
        ...
        ax1 = fig.add_subplot()
        la, = plot(ap_filt_s,sig_filt_s,'-o')
        ...
        la.set_xdata( ap_filt_s)
        la.set_ydata(sig_filt_s)
...
#def lcplot(time,target,comp,sky,fwhm_vec,cstring):
    global iline, icts, l1, l2, l3, l4, l5
    ratio = target/comp
    ...
    if is_first_iter:
        ...
        #l1, = ax1.plot(time,ratio_norm,'o')
        l1.set_xdata(time)
        l1.set_ydata(ratio_norm)
        #l2, = plot(time_smooth[0:-nfilt],ratio_norm_smooth[:-nfilt],'o')
        l2.set_xdata(time_smooth[0:-nfilt])
        l2.set_ydata(ratio_norm_smooth[:-nfilt])
        #l3, = plot(time,comp_norm,'o')
        l3.set_xdata(time)
        l3.set_ydata(comp_norm)
        #pl1 =ax4.plot(time,sky_norm,'o',label = 'Sky')
...
def main(args,is_first_iter):
    ...
    applot(args.fap_pdf, aplist,sigvec,apmin,cstring,is_first_iter)   
    ...
    if comp_ysig <= sigmin:
        fwhm_vec = fwhm_fit2(aplist,comps[:,:,ncmin],is_first_iter)   # FWHM of composite comparison star
    else:
        fwhm_vec = fwhm_fit2(aplist,targs,is_first_iter)   # FWHM of target star
